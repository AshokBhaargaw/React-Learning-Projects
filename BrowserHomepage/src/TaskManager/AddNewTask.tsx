...
export default function AddNewTask() {
  ...
  const addTaskHandler = () => {
    if (input) {
      ...
      inputRef.current &&
        (inputRef.current.placeholder = "Please add task title first...");
    }
  };
  ...
  // on enter click add task
  const enterAndctrlEnterHandler = (e: React.KeyboardEvent) => {
    if (e.key === "Enter") {
      if (e.ctrlKey) {
        setAddDesc(true);
      } else {
      ...
      {addingTask ? (
        <div className="w-full flex justify-end">
          <input
            ...
            ref={inputRef}
            autoFocus
            onKeyDown={enterAndctrlEnterHandler}
